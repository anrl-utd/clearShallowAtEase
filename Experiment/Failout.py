...
class Failout(Layer):
    ...
    # Arguments
        survivability: float between 0 and 1. Probability of node failure.
        seed: A Python integer to use as random seed.
    """
    def __init__(self, survivability, seed=None, **kwargs):
        super(Failout, self).__init__(**kwargs)
        self.seed = seed
        self.survivability = K.variable(survivability)
    ...
    def call(self, inputs, training=None):
        ...
        condition = K.switch(K.greater(rand, self.survivability),fail(inputs),inputs)
